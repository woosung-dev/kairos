...
class OnboardingService:
    ...
    async def increment_step(self, user_id: UUID, target_step: int) -> None:
        ...
        Sprint 29 R1 (auth-cache): step 이 실제 변경되면 User cache(auth_user_id 키, 60s TTL)
        ...
        no-op(이미 advance 됨)이면 auth_user_id=None 이라 무효화도 skip 한다.
        """
        auth_user_id = await self._repo.increment(user_id, target_step)
