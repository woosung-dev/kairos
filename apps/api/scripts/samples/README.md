...
>
> **파일은 git ignore** (개인 음성 데이터). `.gitignore`에 `apps/api/scripts/samples/*.webm` `*.mp4` 등록되어 있음.
...
`apps/api/scripts/record_samples.sh` — ffmpeg + avfoundation 인터랙티브 녹음. 7 sample 일괄 ~15분.

```bash
chmod +x apps/api/scripts/record_samples.sh
bash apps/api/scripts/record_samples.sh           # 일반 (5min 포함)
bash apps/api/scripts/record_samples.sh --skip-long  # 5min skip (~10분)
bash apps/api/scripts/record_samples.sh --silent-only  # 무음 1개만 (smoke)
```
...
```bash
ls -lh apps/api/scripts/samples/
...
`apps/api/scripts/samples/*.webm` `*.mp4` `*.wav` 패턴 ignore — 본 README + `.gitkeep`만 commit.
