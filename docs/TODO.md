...
- [x] 온보딩 템플릿 프로젝트 자동 시딩 — `create_workspace()` 가 🚀 시작하기 · 💡 아이디어 · 📋 회의록 3개 프로젝트 자동 생성 + OnboardingBanner 안내 개편 (이번 커밋)
...
- [ ] 사이드바 소스 트리 + Source Viewer 실 API 연동 (mock 잔재 제거 — 플랜 작업 3/3)
- [ ] FE ↔ BE 실제 프로덕션 E2E 시나리오 검증 (업로드→STT→Inbox→프로젝트→RAG)
