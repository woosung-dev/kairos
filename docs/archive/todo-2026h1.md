# TODO 아카이브 (2026-08-16 분할 이전 원본)

> **역사 기록이다 — 소급 수정하지 않는다.**
> 2026-08-16 에 `docs/TODO.md` 를 열린 항목만 남기도록 줄이면서, 분할 직전 원본 전체를 여기 보존했다.
> 경로 표기 `apps/backend/` 는 `apps/api/` 로 읽는다 (ADR-030 D2).
> 완료 이력의 정본은 git log + `docs/REFACTORING-BACKLOG.md` 다 (AGENTS.md §4).
