...
| `POST` | `/api/v1/workspaces/{workspace_id}/integrations/google-drive/documents` | owner | `202 Accepted` `{ syncRunId }` | 선택 file IDs와 `projectId`를 받아 BackgroundTask import 시작. `404` 연결 또는 같은 workspace의 Project 없음, `403` owner 아님. 지원하지 않는 MIME은 pipeline이 문서별 `failed`로 기록한다. |
