...
FROM meetings JOIN meeting_project_links ON meeting_project_links.meeting_id = meetings.id 
WHERE meetings.workspace_id = %(workspace_id_1)s::UUID AND meeting_project_links.project_id = %(project_id_1)s::UUID AND (NOT (EXISTS (SELECT * 
