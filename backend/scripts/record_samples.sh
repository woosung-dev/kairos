...
#   bash backend/scripts/record_samples.sh
#   bash backend/scripts/record_samples.sh --device 0   # 마이크 device index 명시
#   bash backend/scripts/record_samples.sh --skip-long  # 5min 큰 sample skip
#   bash backend/scripts/record_samples.sh --silent-only  # silent 1개만
#
# 출력: backend/scripts/samples/*.webm + *.mp4
# 후속: cd backend && uv run python scripts/sprint15_day0_spike.py
...
echo ""
echo "===== 다음 단계 ====="
echo "  cd backend && uv run python scripts/sprint15_day0_spike.py"
echo "  # 결과 → docs/dev-log/sprint-15-cost-spike.md §3.1 / §3.3 audio 결과 paste"
