...
| I-14 | **Pydantic V2 + 100% async + SQLModel typed query (Sprint 20 BL-054 갱신, 2026-05-18)**: `.dict()` 대신 `.model_dump()`, `BaseSettings`는 `pydantic_settings`에서 import. **session.exec / execute allowlist** (manifest `docs/dev-log/2026-05-18-bl054-execute-manifest.md`): (G1) typed scalar select → `session.exec(stmt).all() / .first() / .one_or_none() / .one()` 강제. (G2/G4) raw text() / multi-column tuple result → `session.execute()` 유지 (SM exec 가 text/dialect 미수용). (G3-convert) DML w/o rowcount → `session.exec()` 변환. (G3-keep) DML w/ `.rowcount` 사용 → `session.execute()` 유지 (rowcount contract preservation). (G3-keep-dialect) `pg_insert(...).on_conflict_do_nothing()` 등 SA dialect insert → `session.execute()` 영구 유지 (SQLModel 미 re-export). | code review |
